...
export class AddressesService {
  ...
  async update(user_id, id, data) {
    ...
    if (!res) throw new HttpException(messages.addressNotFound, HttpStatus.NOT_FOUND);
    return res;
  }
  async getById(user_id, id) {
    const data = await this.Addresses.findOne({where: {user_id, id}});
    if (!data) throw new HttpException(messages.addressNotFound, HttpStatus.NOT_FOUND)
    return data;
  }
